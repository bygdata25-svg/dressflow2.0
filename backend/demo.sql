...
BEGIN
    ...
    FOR i IN 1..10 LOOP
        ...
        INSERT INTO sale_items (
            id, tenant_id, sale_id, item_type, item_id, description,
            quantity, unit_price, line_total, currency, created_at, updated_at
        )
        SELECT
            gen_random_uuid(),
            v_tenant_id,
            ...
            'DRESS',
            v_dress_ids[((i - 1) % array_length(v_dress_ids, 1)) + 1],
            (SELECT name FROM dresses WHERE id = v_dress_ids[((i - 1) % array_length(v_dress_ids, 1)) + 1]),
            1,
            CASE WHEN i IN (4, 8) THEN v_amount - 50 ELSE v_amount END,
            CASE WHEN i IN (4, 8) THEN v_amount - 50 ELSE v_amount END,
            'USD',
        ...
        INSERT INTO sale_payments (
            id, tenant_id, sale_id, payment_method, amount, currency, created_at, updated_at
        )
        SELECT
            gen_random_uuid(),
            v_tenant_id,
    ...
    FOR i IN 1..10 LOOP
        ...
        SELECT
            ...
            'ACCESSORY',
            v_accessory_ids[((i - 1) % array_length(v_accessory_ids, 1)) + 1],
            (SELECT name FROM accessories WHERE id = v_accessory_ids[((i - 1) % array_length(v_accessory_ids, 1)) + 1]),
            v_qty,
            v_unit_price,
            CASE WHEN i IN (3, 9) THEN v_amount - 2500 ELSE v_amount END,
            'ARS',
    ...
    SELECT
        ...
        'DRESS',
        v_dress_ids[6],
        (SELECT name FROM dresses WHERE id = v_dress_ids[6]),
        1,
        1850,
        1850,
        'USD',
    ...
    SELECT
        ...
        'ACCESSORY',
        v_accessory_ids[1],
        (SELECT name FROM accessories WHERE id = v_accessory_ids[1]),
        1,
        20500,
        20500,
        'ARS',
