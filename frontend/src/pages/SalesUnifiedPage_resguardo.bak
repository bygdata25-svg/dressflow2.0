...
export default function SalesUnifiedPage() {
  ...
  const addPayment = () => {
    ...
    if (amount <= 0) {
      alert("Ingresá un monto de pago mayor a 0.");
      return;
    }
  ...
  const handleSubmitSale = async () => {
    if (!form.customer_id) {
      alert("Debés seleccionar un cliente.");
      return;
    }

    if (!form.items.length) {
      alert("Debés agregar al menos un ítem.");
      return;
    }
    ...
    if (payments.length === 0) {
      alert("Debés cargar al menos una forma de pago.");
      return;
    }
    ...
    if (invalidItem) {
      ...
      alert("Hay ítems incompletos o inválidos.");
      return;
    }
    ...
    if (invalidPayment) {
      ...
      alert("Hay pagos incompletos o inválidos.");
      return;
    }
    ...
    if (needsExchangeRate && exchangeRateNumber <= 0) {
      alert("Debés ingresar un tipo de cambio válido para convertir entre ARS y USD.");
      return;
    }
    ...
    try {
      ...
      await api.post("/sales-unified", payload);
      ...
      alert(
        typeof (error as any)?.response?.data?.detail === "string"
          ...
          : JSON.stringify((error as any)?.response?.data || { message: "No se pudo guardar la venta." }, null, 2)
      );
    } finally {
  ...
  const handleSubmitCustomer = async () => {
    if (!customerForm.first_name.trim()) {
      alert("Debés ingresar el nombre del cliente.");
      return;
    }
